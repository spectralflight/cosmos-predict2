...
class Processor:  # TODO (maxzhaoshuol): This should be inherit AutoProcessor, instead of using self.processor?
    ...
    def __init__(self, name="Qwen/Qwen2.5-VL-3B-Instruct", cache_dir=None):
        ...
        if name in ["Qwen/Qwen2.5-VL-7B-Instruct", "Qwen/Qwen2.5-VL-3B-Instruct", "Qwen/Qwen2-VL-2B-Instruct"]:
            if cache_dir is None:
                ...
                cache_dir = os.path.join(imaginaire_cache_dir, name)
            ...
            with lock:
                ...
                if os.path.exists(cache_dir) and os.path.exists(os.path.join(cache_dir, "tokenizer.json")):
                    log.info(f"Tokenizer already exists in cache {cache_dir}, skipping download")
                else:
                    log.info("Downloading tokenizer to local cache...")
                    backend_args = {
                        "backend": "s3",
                        "path_mapping": None,
                    ...
                    log.info(f"Successfully downloaded tokenizer to local cache {cache_dir}")
        ...
        self.eos_id = self.processor.tokenizer.eos_token_id  # TODO (maxzhaoshuol): should be self.eos_token_id??
